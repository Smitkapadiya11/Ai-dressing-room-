...
function buildPrompt({ garment, bodyRead }) {
  const anchor = bodyRead
    ? `\nConfirmed build of the person in the first image — match exactly, this is a measurement, not a suggestion:\n${bodyRead}\n`
    : "";

  return `Virtual garment fitting. First image: a real person, photographed as-is, facing whichever way they happen to be facing — front, side, three-quarter, back. Second image: a garment.

Edit the first image so the person wears the garment from the second image. Keep the exact same angle and direction the person is facing — do not turn them toward camera or reframe. Nothing else changes.
${anchor}
LOCKED — copy exactly from the first image, whatever it shows:
face, skin tone, body shape and size, pose, hand and foot position, hair, background, floor, lighting direction, camera angle.

COPIED — match exactly from the second image:
...
The garment drapes over the real body in the first image — wider on a broader person, shorter on a shorter one. Never resize, slim, or idealise the body to fit the garment. Result must read as an unretouched photograph: no seams, no warped edges, no lighting mismatch between garment and skin.`;
}
