...
const IMAGE_MODEL_CHAINS = {
  ...
  // Confirmed against OpenAI's published pricing page (Sep 2026 image
  // generation table) and against this project's own real usage:
  //   - "gpt-image-2.5" (bare) is NOT a real model — the actual names are
  //     gpt-image-2.5-sunburst and gpt-image-2.5-flare. The old chain led
  //     with the bare name, so it 404'd on literally every request before
  //     falling through — wasted latency, not wasted money (a 404 fails
  //     before billing), but still worth fixing.
  //   - gpt-image-2 is what ended up running every real fitting so far and
  //     is the one this whole config has actually been validated against,
  //     so it leads. sunburst/flare/1.5 sit right behind it at the exact
  //     same published price ($8 in / $30-32 out per 1M tokens) — pure
  //     upside to try if gpt-image-2 is ever unavailable, never a downside.
  //   - gpt-image-1 stays last as the "slightly higher, not much" fallback:
  //     $10/$40 vs $8/$30, about 30% more, and the one name in this whole
  //     chain guaranteed to exist on every account.
  demo: (
    process.env.OPENAI_IMAGE_MODEL_DEMO ||
    "gpt-image-2,gpt-image-2.5-sunburst,gpt-image-2.5-flare,gpt-image-1.5,gpt-image-1"
  ).split(","),
};
...
// MEASURED, not estimated, as of the last update: 3 real demo-tier
// fittings on gpt-image-2 (medium quality) cost $0.12 total on the
// OpenAI dashboard — $0.04/image, ~Rs 3.80 at ~95.6 INR/USD. That
// replaces the earlier ~4x-surcharge guess (Rs 15.68), which turned out
// to be roughly 4x too high. If PROVIDER, RESULT_TIER, or the model
// chain above changes, re-measure rather than trust this number blind —
// it is only as good as the fittings it was measured against.
...
const COST_INR_BY_TIER = {
  ...
  demo: 3.8, // gpt-image-2, medium — measured from real billing, see above
};
