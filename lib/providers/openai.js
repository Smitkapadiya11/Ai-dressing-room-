...
// "low" is the demo default. "medium"/"high" are available and cost more
// (roughly 2x and 8x respectively, per OpenAI's published per-quality
// token counts) — set OPENAI_IMAGE_QUALITY_DEMO=medium only for a final
// warm-the-cache pass before a meeting, not for routine use. The actual
// per-image cost at any quality is computed live from real usage tokens,
// see costInrFromUsage below — nothing here is a price guess anymore.
...
// FALLBACK ONLY — used solely if OpenAI ever omits `usage` from a response
// (see costInrFromUsage below, which is what actually prices every real
// call from real billed tokens). A flat number here can't tell a
// two-reference-image edit with a long prompt (this app, every request)
// apart from a bare one-line text-to-image call, which is the gap that
// made the old flat guess unreliable. Rs 4.27/image is OpenAI's published
// rate for gpt-image-2.5-sunburst at "low" on a generic image — an
// estimate, not a number measured from this account's own billing, and
// only ever used if a future model's response is missing token counts.
...
// THE REAL PRICE — every gpt-image call returns a `usage` object counting
// exactly how many text-input, image-input and output tokens it actually
// billed, broken out because OpenAI charges each at a different rate. That
// makes the true cost of any one call computable from the response itself,
// instead of guessed from a flat per-model number that can't see how many
// reference images or how long the prompt was. This is what fixed the
// COST_INR_BY_TIER guess above being off by ~3x on a real edit.
//
// Rates are OpenAI's published $/1M-token prices (Sep 2026 pricing page).
const USD_TO_INR = 95.6; // matches the rate this file's earlier measurements used
const PRICE_PER_1M_USD = {
  // gpt-image-2, gpt-image-2.5-sunburst, gpt-image-2.5-flare, gpt-image-1.5
  // all publish the identical rate.
  v2: { text: 5, image: 8, output: 30 },
  // Legacy, ~2x the image-input rate and higher output rate than v2.
  legacy: { text: 5, image: 10, output: 40 },
  // gpt-image-1-mini — the "test" tier model.
  mini: { text: 2, image: 2.5, output: 8 },
};
function priceFamily(model) {
  const m = String(model || "");
  if (m.includes("mini")) return PRICE_PER_1M_USD.mini;
  if (/^gpt-image-1(?!\.5)/.test(m)) return PRICE_PER_1M_USD.legacy; // gpt-image-1, not gpt-image-1.5
  return PRICE_PER_1M_USD.v2;
}

// null, not a guess, if the response didn't include usage — the caller
// falls back to the flat COST_INR_BY_TIER estimate rather than fabricate
// a number from missing data.
function costInrFromUsage(model, usage) {
  ...
  if (!textTokens && !imageTokens && !outputTokens) return null;

  const rate = priceFamily(model);
  const usd =
    (textTokens / 1e6) * rate.text + (imageTokens / 1e6) * rate.image + (outputTokens / 1e6) * rate.output;
...
async function editImage(model, quality, personJpeg, garmentJpeg, prompt) {
  ...
  const res = await fetch(`${API_ROOT}/images/edits`, {
    // Under the route's 120s maxDuration, so she gets a clear error, not a hang.
  ...
  return { jpeg: Buffer.from(b64, "base64"), usage: data?.usage || null };
}

// `model` / `quality` pin a specific OpenAI option from the engine picker;
// the tier chain stays behind it as the fallback.
export async function generateImage({ personJpeg, garmentJpeg, prompt, tier, model: pinned, quality: pinnedQuality }) {
  const tierChain = IMAGE_MODEL_CHAINS[tier] || IMAGE_MODEL_CHAINS.demo;
  const chain = pinned ? [pinned, ...tierChain.filter((m) => m !== pinned)] : tierChain;
  const quality = pinnedQuality || IMAGE_QUALITY_BY_TIER[tier] || IMAGE_QUALITY_BY_TIER.demo;
  const { result, model } = await withModelFallback(chain, (m) => editImage(m, quality, personJpeg, garmentJpeg, prompt));
  const costInr = costInrFromUsage(model, result.usage) ?? COST_INR_BY_TIER[tier] ?? COST_INR_BY_TIER.demo;
  return { jpeg: result.jpeg, costInr };
}
...
export async function generateFromText({ prompt, tier }) {
  ...
  const { result, model } = await withModelFallback(chain, (m) => generateFrom(m, quality, prompt));
  const costInr =
    costInrFromUsage(model, result.usage) ??
    TEXT_TO_IMAGE_COST_INR_BY_TIER[tier] ??
    TEXT_TO_IMAGE_COST_INR_BY_TIER.demo;
  return { jpeg: result.jpeg, costInr };
}
