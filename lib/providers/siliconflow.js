...
// After a 402/403 (no balance) the FLUX engines drop out of the picker and
// requests go straight to the house engine for a while, instead of paying
// a failed round-trip on every fitting.
const EMPTY_BALANCE_BACKOFF_MS = 10 * 60_000;
let emptyUntil = 0;

export const isConfigured = () => Boolean(process.env.SILICONFLOW_API_KEY) && Date.now() >= emptyUntil;
...
export async function generateImage({ personJpeg, garmentJpeg, prompt, model }) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 402 || res.status === 403) emptyUntil = Date.now() + EMPTY_BALANCE_BACKOFF_MS;
